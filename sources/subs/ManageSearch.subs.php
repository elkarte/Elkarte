...
/**
 * Creates and outputs the Manticore configuration file
 *
 * @package Search
 */
function createManticoreConfig()
{
	global $db_server, $db_name, $db_user, $db_passwd, $db_prefix, $modSettings;
	...
	// Check paths are set, if not use some defaults
	$modSettings['manticore_data_path'] = empty($modSettings['manticore_data_path']) ? '/var/manticore/data' : $modSettings['manticore_data_path'];
	$modSettings['manticore_log_path'] = empty($modSettings['manticore_log_path']) ? '/var/manticore/log' : $modSettings['manticore_log_path'];
	$prefix = (!empty($modSettings['manticore_index_prefix']) ? $modSettings['manticore_index_prefix'] : 'elkarte');

	// Output our minimal configuration file to get them started
	echo '#
# Manticore configuration file (manticore.conf), configured for ElkArte
#
# This is the minimum needed clean, simple, functional
#
# By default the location of this file would probably be:
# /usr/local/etc/manticore.conf or /etc/manticoresearch/manticore.conf
#
...
source ', $prefix, '_source
{
	...
	sql_query			= \
		SELECT \
			m.id_msg, m.id_topic, m.id_board, CASE WHEN m.id_member = 0 THEN 4294967295 ELSE m.id_member END AS id_member, \
			m.poster_time, m.body, m.subject, t.num_replies + 1 AS num_replies, t.num_likes, t.is_sticky, \
			1 - ((m.id_msg - t.id_first_msg) / NULLIF(t.id_last_msg - t.id_first_msg, 0)) AS position, \
			CEILING(10 * ( \
				CASE WHEN m.id_msg < 0.6 * s.value THEN 0 ELSE (m.id_msg - 0.6 * s.value) / (0.4 * s.value) END * ' . $weight['age'] . ' + \
				CASE WHEN t.num_replies < 50 THEN t.num_replies / 50 ELSE 1 END * ' . $weight['length'] . ' + \
				CASE WHEN m.id_msg = t.id_first_msg THEN 1 ELSE 0 END * ' . $weight['first_message'] . ' + \
...
index ', $prefix, '_base_index
{
	...
	blend_mode			= trim_head | trim_none
	source				= ', $prefix, '_source
	path				= ', $modSettings['manticore_data_path'], '/', $prefix, '_base.index', (empty($modSettings['manticore_stopword_path']) ? '' : '
	stopwords			= ' . $modSettings['manticore_stopword_path']), (empty($modSettings['manticore_wordform_path']) ? '' : '
	wordforms			= ' . $modSettings['manticore_wordform_path']), '
	min_word_len		= 2
	charset_table		= 0..9, A..Z->a..z, _, a..z, U+451->U+435, U+401->U+435, U+410..U+42F->U+430..U+44F, U+430..U+44F
	ignore_chars		= U+AD
	morphology			= lemmatize_en_all, metaphone
}

index ', $prefix, '_delta_index : ', $prefix, '_base_index
{
	source			= ', $prefix, '_delta_source
	path			= ', $modSettings['manticore_data_path'], '/', $prefix, '_delta.index
}
...
indexer
{
	mem_limit		 = ', (empty($modSettings['manticore_indexer_mem']) ? 512 : (int) $modSettings['manticore_indexer_mem']), 'M
...
searchd
{
	listen			= ', (empty($modSettings['manticore_searchd_port']) ? 9306 : (int) $modSettings['manticore_searchd_port']), ':mysql41
...
function createSearchIndex($start, $messages_per_batch)
{
	...
	else
	{
		...
		while (time() < $stop)
		{
			...
			updateSettings(['search_custom_index_resume' => serialize(['resume_at' => $start])]);
		}
...
function removeCommonWordsFromIndex($start)
{
	...
	$stop_words = $start === 0 || empty($modSettings['search_stopwords']) ? [] : explode(',', $modSettings['search_stopwords']);
	$stop = time() + 3;
	...
	return [$start, $complete, $percentage];
}
