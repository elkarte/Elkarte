...
function template_build_poster_div($message, $ignoring)
{
	...
	if (!empty($message['poster_area_hidden']))
	{
		foreach ($message['poster_area_hidden'] as $item)
		{
			if (substr($item, 0, 9) == 'separator')
			{
				$poster_div .= '
									<li class="listlevel2 ' . $item . '"></li>';
				continue;
			}
			if ($item == 'profile' && !empty($message['hidden_fields']['profile']))
			{
				// /me is tired of writing down all the complete reference...
				$shortcut = &$message['hidden_fields']['profile'];
				$shortcut['text'] = '
										<ol>';
				foreach ($shortcut['values'] as $profile_field)
					$shortcut['text'] .= '
							<li class="' . $profile_field . '">
								' . (isset($shortcut[$profile_field]['url']) ? '<a href="' . $shortcut[$profile_field]['url'] . '"' . (isset($shortcut[$profile_field]['options']) ? $shortcut[$profile_field]['options'] : '') . '>' : '') . 
								(isset($shortcut[$profile_field]['img']) ? $shortcut[$profile_field]['img'] : '') . 
								$shortcut[$profile_field]['text'] . 
								(isset($shortcut[$profile_field]['indicator']) ? $shortcut[$profile_field]['indicator'] : '') . 
								(isset($shortcut[$profile_field]['url']) ? '
								</a>' : '') . '
							</li>';
				$shortcut['text'] .= '
										</ol>';
			...
			$poster_div .= '
							<li class="listlevel2 ' . $item . '">
								' . (isset($message['hidden_fields'][$item]['url']) ? '<a class="linklevel2' . (isset($message['hidden_fields'][$item]['class']) ? ' ' . $message['hidden_fields'][$item]['class'] :'') . '" href="' . $message['hidden_fields'][$item]['url'] . '"' . (isset($message['hidden_fields'][$item]['options']) ? $message['hidden_fields'][$item]['options'] : '') . '>' : '') . 
								(isset($message['hidden_fields'][$item]['img']) ? $message['hidden_fields'][$item]['img'] : '') . 
								$message['hidden_fields'][$item]['text'] . 
								(isset($message['hidden_fields'][$item]['indicator']) ? $message['hidden_fields'][$item]['indicator'] : '') . 
								(isset($message['hidden_fields'][$item]['url']) ? '
								</a>' : '') . '
							</li>';
		}
	}
	...
	if (empty($options['hide_poster_area']) && !empty($message['poster_area_visible']))
	{
		foreach ($message['poster_area_visible'] as $item)
			$poster_div .= '
							<li class="listlevel1 ' . $item . '">
								' . (isset($message[$item]['url']) ? '<a class="linklevel1" href="' . $message[$item]['url'] . '"' . (isset($message[$item]['options']) ? $message[$item]['options'] : '') . '">' : '') . 
								(isset($message[$item]['img']) ? $message[$item]['img'] : '') . 
								$message[$item]['text'] . 
								(isset($message[$item]['indicator']) ? $message[$item]['indicator'] : '') . 
								(isset($message[$item]['url']) ? '
								</a>' : '') . '
							</li>';
	}
