...
class Register_Controller extends Action_Controller
{
	...
	public function do_register($verifiedOpenID = false)
	{
		...
		if ($this->_req->getPost('birthdate', 'trim', '') !== '')
			$this->_req->post->birthdate = elk_strftime('%Y-%m-%d', strtotime($this->_req->post->birthdate));
		// Or birthdate parts...
		...
		$agreement->accept($memberID, $user_info['ip'], empty($modSettings['agreementRevision']) ? elk_strftime('%Y-%m-%d', forum_time(false)) : $modSettings['agreementRevision']);

		if (!empty($modSettings['requirePrivacypolicy']))
		{
			$policy = new \PrivacyPolicy($lang);
			$policy->accept($memberID, $user_info['ip'], empty($modSettings['privacypolicyRevision']) ? elk_strftime('%Y-%m-%d', forum_time(false)) : $modSettings['privacypolicyRevision']);
		}
	...
	public function action_agreement()
	{
		...
		{
			$agreement = new Agreement($user_info['language']);
			$agreement->accept($user_info['id'], $user_info['ip'], empty($modSettings['agreementRevision']) ? elk_strftime('%Y-%m-%d', forum_time(false)) : $modSettings['agreementRevision']);
	...
	public function action_privacypol()
	{
		...
		if (isset($this->_req->post->accept_agreement))
		{
			$policy->accept($user_info['id'], $user_info['ip'], empty($modSettings['privacypolicyRevision']) ? elk_strftime('%Y-%m-%d', forum_time(false)) : $modSettings['privacypolicyRevision']);
