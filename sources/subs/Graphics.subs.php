...
function createThumbnail($source, $max_width, $max_height)
{
	...
	$image = new \ElkArte\Graphics\Image($destName . '.tmp');
	$success = $image->resizeImageFile($source, $max_width, $max_height, $format, true);
...
{
	$image = new \ElkArte\Graphics\Image($fileName . '.tmp', null, null, $preferred_format);
	if ($image->resizeImageFile($fileName) === false)
	{
...
function showCodeImage($code)
{
	...
	$gd2 = \ElkArte\Graphics\Gd2::canUse(true);
