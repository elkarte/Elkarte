...
class PermissionsTest extends ElkArteCommonSetupTest
{
	...
	public function testIntegrationIllegalGuestPermissions()
	{
		...
		$permissionsObject = new Permissions();
		$illegal_guest_permissions = $permissionsObject->getIllegalGuestPermissions();
		testIntegrationIGP($illegal_guest_permissions);
...
function testIntegrationIGP(&$illegal_guest_permissions)
{
	...
	if (($key = array_search('pm_read', $illegal_guest_permissions)) !== false)
	{
		unset($illegal_guest_permissions[$key]);
	}

	// remove this
	if (($key = array_search('no_bla_4_you', $context['non_guest_permissions'])) !== false)
	{
		unset($context['non_guest_permissions'][$key]);
	}

	if (($key = array_search('issue_warning', $context['non_guest_permissions'])) !== false)
	{
		unset($context['non_guest_permissions'][$key]);
	}
