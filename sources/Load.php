...
function loadTemplate($template_name, $style_sheets = array(), $fatal = true)
{
	...
	if ($loaded)
	{
		if ($db_show_debug === true)
			Debug::add('templates', $template_name . ' (' . basename($template_dir) . ')');
...
function loadSubTemplate($sub_template_name, $fatal = false)
{
	...
	if ($db_show_debug === true)
		Debug::add('sub_templates', $sub_template_name);
	...
	else
	{
		...
		foreach ($filenames as $filename)
		{
			...
			if (!empty($filename))
			{
				...
				if ($db_show_debug === true)
					Debug::add($params['debug_index'], $params['basename'] . '(' . (!empty($params['local']) ? (!empty($params['url']) ? basename($params['url']) : basename($params['dir'])) : '') . ')');
			}
...
function loadLanguage($template_name, $lang = '', $fatal = true, $force_reload = false)
{
	...
	if ($db_show_debug === true)
		Debug::add('language_files', $template_name . '.' . $lang . ' (' . $theme_name . ')');
