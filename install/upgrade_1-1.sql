...
if ((!isset($modSettings['smfVersion']) || $modSettings['smfVersion'] <= '1.1 RC1') && empty($modSettings['dont_repeat_theme']))
{
	...
	if ($db->num_rows($theme_request) == 0)
	{
		...
		$request = upgrade_query("
			...
			FROM {$db_prefix}themes");
		list ($ID_OLD_THEME) = $db->fetch_row($request);
		$db->free_result($request);
...
$upgradeBanTable = $db->num_rows($request) == 1;
$db->free_result($request);
...
if (!isset($modSettings['permission_enable_deny']))
{
	...
	$request = upgrade_query("
		...
		WHERE addDeny = 0
		LIMIT 1");
	$disable_deny_permissions = $db->num_rows($request) == 0;
	$db->free_result($request);
...
if (!isset($modSettings['permission_enable_postgroups']))
{
	...
	$request = upgrade_query("
		...
		WHERE mg.ID_GROUP = p.ID_GROUP
			AND mg.minPosts != -1
		LIMIT 1");
	$disable_postgroup_permissions &= $db->num_rows($request) == 0;
	$db->free_result($request);
...
if (!isset($modSettings['permission_enable_by_board']))
{
	...
	$request = upgrade_query("
		...
		LIMIT 1");
	$enable_by_board = $db->num_rows($request) == 1 ? '1' : '0';
	$db->free_result($request);
...
list ($numIndexedWords) = $db->fetch_row($request);
$db->free_result($request);
if ($numIndexedWords == 0 || isset($_GET['lt']))
{
	...
	list ($maxTopics) = $db->fetch_row($request);
	$db->free_result($request);
	...
	while ($_GET['lt'] <= $maxTopics)
	{
		...
		while ($row = $db->fetch_assoc($request))
		{
			foreach (text2words($row['subject']) as $word)
				$inserts[] = "'" . $db->escape_string($word) . "', $row[ID_TOPIC]";
		}
		$db->free_result($request);
...
if ($upgradeLogTable)
{
	$request = upgrade_query("
		SELECT MAX(ID_MSG)
		FROM {$db_prefix}messages");
	list ($maxMsg) = $db->fetch_row($request);
	$db->free_result($request);
	...
	if ($_GET['m'] === 0)
	{
		...
		$request = upgrade_query("
			...
			FROM {$db_prefix}messages
			WHERE ID_MSG = $maxMsg");
		list ($maxPosterTime) = $db->fetch_row($request);
		$db->free_result($request);
	...
	else
	{
		...
		$request = upgrade_query("
			...
			FROM {$db_prefix}messages
			WHERE ID_MSG < $_GET[m]");
		list ($lower_limit) = $db->fetch_row($request);
		$db->free_result($request);
...
if ($upgradeBoardsTable)
{
	$request = upgrade_query("
		SELECT MAX(ID_BOARD)
		FROM {$db_prefix}boards");
	list ($maxBoard) = $db->fetch_row($request);
	$db->free_result($request);
...
$request = upgrade_query("
	...
	FROM {$db_prefix}messages
	LIKE 'body'");
$body_row = $db->fetch_assoc($request);
$db->free_result($request);
