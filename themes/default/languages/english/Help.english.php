...
$helptxt['minify_css_js'] = 'This combines multiple CSS and Javascript files on a per page basis. It also removes whitespace and comments from the files to reduce their size.  The combined and minimized files are saved so any further requests can be served with those saved files.  Note that the first time a compilation is created, there will be a slight delay on that page load in order to create the file (this will also happen after the cache is cleared)';
