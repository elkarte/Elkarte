...
class Calendar_Event
{
	...
	public function update($options)
	{
		...
		$eventOptions = array(
			'title' => Util::substr($options['evtitle'], 0, 100),
			'span' => $span,
			'start_date' => elk_strftime('%Y-%m-%d', mktime(0, 0, 0, (int) $options['month'], (int) $options['day'], (int) $options['year'])),
	...
	public function load($options, $member_id)
	{
		...
		if ($this->isNew())
		{
			...
			$event['last_day'] = (int) elk_strftime('%d', mktime(0, 0, 0, $event['month'] == 12 ? 1 : $event['month'] + 1, 0, $event['month'] == 12 ? $event['year'] + 1 : $event['year']));
		}
		else
		{
