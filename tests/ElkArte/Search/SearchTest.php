...
class SearchTest extends TestCase
{
	...
	{
		$this->assertIsInt($this->member_full_access, 'AdminController member not found ' . $this->member_full_access);
		$this->loadUser($this->member_full_access);

		$topics = $this->_performSearch();
		$this->assertCount(2, $topics, 'AdminController search results not correct, found ' . count($topics) . ' instead of 2');
	}
