...
class ElkArteInstallWeb extends ElkArteWebSupport
{
	...
	public function testInstall()
	{
		...
		$this->assertStringContainsString('Settings.php and Settings_bak.php will be created in the default directory', $this->byCssSelector('#main_screen form .information')->text(), $this->source());

		// Warning gone
		$this->prepareSettings();
		$this->url('install/install.php');
		$this->assertEquals('ElkArte Installer', $this->title(), 'step 2');
		$this->assertStringNotContainsString('Settings.php and Settings_bak.php will be created in the default directory', $this->byCssSelector('#main_screen form')->text(), $this->source());

		// Let's start
		$this->clickit('#contbutt');
		$this->assertEquals('Database Server Settings', $this->byCssSelector('#main-content')->text(), $this->source());
		...
		$this->assertEquals('Forum Settings', $this->byCssSelector('#main-content')->text(), $this->source());
		$this->byId('forum_settings')->submit();
		sleep(15);
		$this->assertEquals('Populated Database', $this->byCssSelector('#main-content')->text(), $this->source());

		// All that is left is to create our admin account
		$this->byId('populate_db')->submit();
		$this->assertEquals('Create Your Account', $this->byCssSelector('#main-content')->text(), $this->source());
		...
		$this->assertStringContainsString('Congratulations', $this->byCssSelector('#main-content')->text(), $this->source());
