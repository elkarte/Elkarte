...
function addKarma($id_executor, $id_target, $direction)
{
	...
	$db->replace(
		'{db_prefix}log_karma',
		['action' => 'int', 'id_target' => 'int', 'id_executor' => 'int', 'log_time' => 'int'],
		[$direction, $id_target, $id_executor, time()],
		['id_target', 'id_executor']
	);

	// Change by one.
	require_once(SUBSDIR . '/Members.subs.php');
	updateMemberData($_REQUEST['uid'], [$direction == 1 ? 'karma_good' : 'karma_bad' => '+']);
}
...
function updateKarma($id_executor, $id_target, $direction)
{
	...
	if ($direction == 1)
	{
		updateMemberData($_REQUEST['uid'], ['karma_good' => '+', 'karma_bad' => '-']);
	}
	else
	{
		updateMemberData($_REQUEST['uid'], ['karma_bad' => '+', 'karma_good' => '-']);
	}
}
