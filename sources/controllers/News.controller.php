		...
		foreach ($members as $member)
		{
			...
			elseif ($xml_format === 'atom')
				$data[] = array(
					'title' => cdata_parse($member['real_name']),
					'link' => $scripturl . '?action=profile;u=' . $member['id_member'],
					'published' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $member['date_registered']),
					'updated' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $member['last_login']),
		...
		foreach ($results as $row)
		{
			...
			elseif ($xml_format === 'atom')
			{
				$data[] = array(
					...
					'published' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $row['poster_time']),
					'modified' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', empty($row['modified_time']) ? $row['poster_time'] : $row['modified_time']),
		...
		foreach ($results as $row)
		{
			...
			elseif ($xml_format === 'atom')
			{
				$data[] = array(
					...
					'published' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $row['poster_time']),
					'updated' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', empty($row['modified_time']) ? $row['poster_time'] : $row['modified_time']),
	...
	public function action_xmlprofile($xml_format)
	{
		...
		elseif ($xml_format === 'atom')
		{
			$data[] = array(
				...
				'published' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $user_profile[$profile['id']]['date_registered']),
				'updated' => elk_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $user_profile[$profile['id']]['last_login']),
