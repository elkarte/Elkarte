...
function getEventRange($low_date, $high_date, $use_permissions = true, $limit = null)
{
	...
	while ($row = $db->fetch_assoc($result))
	{
		...
		for ($date = $start_date; $date <= $end_date; $date += 86400)
		{
			...
			if (elk_strftime('%Y-%m-%d', $date) == $lastDate)
				$date += 3601;
			$lastDate = elk_strftime('%Y-%m-%d', $date);

			// If we're using permissions (calendar pages?) then just ouput normal contextual style information.
			if ($use_permissions)
				$events[elk_strftime('%Y-%m-%d', $date)][] = array(
					'id' => $row['id_event'],
					'title' => $row['title'],
...
{
	return array(
		'day' => (int) elk_strftime('%d', forum_time()),
		'month' => (int) elk_strftime('%m', forum_time()),
		'year' => (int) elk_strftime('%Y', forum_time()),
		'date' => elk_strftime('%Y-%m-%d', forum_time()),
	);
}
...
function getCalendarGrid($month, $year, $calendarOptions)
{
	...
	$month_info = array(
		'first_day' => array(
			'day_of_week' => (int) elk_strftime('%w', mktime(0, 0, 0, $month, 1, $year)),
			'week_num' => (int) elk_strftime('%U', mktime(0, 0, 0, $month, 1, $year)),
			'date' => elk_strftime('%Y-%m-%d', mktime(0, 0, 0, $month, 1, $year)),
		),
		'last_day' => array(
			'day_of_month' => (int) elk_strftime('%d', mktime(0, 0, 0, $month == 12 ? 1 : $month + 1, 0, $month == 12 ? $year + 1 : $year)),
			'date' => elk_strftime('%Y-%m-%d', mktime(0, 0, 0, $month == 12 ? 1 : $month + 1, 0, $month == 12 ? $year + 1 : $year)),
		),
		'first_day_of_year' => (int) elk_strftime('%w', mktime(0, 0, 0, 1, 1, $year)),
		'first_day_of_next_year' => (int) elk_strftime('%w', mktime(0, 0, 0, 1, 1, $year + 1)),
	);
...
function getCalendarWeek($month, $year, $day, $calendarOptions)
{
	...
	$calendarOptions['start_day'] = empty($calendarOptions['start_day']) ? 0 : (int) $calendarOptions['start_day'];
	$day_of_week = (int) elk_strftime('%w', mktime(0, 0, 0, $month, $day, $year));
	if ($day_of_week != $calendarOptions['start_day'])
	{
		...
		$day = (int) elk_strftime('%d', $new_timestamp);
		$month = (int) elk_strftime('%m', $new_timestamp);
		$year = (int) elk_strftime('%Y', $new_timestamp);
	}
	...
	$calendarGrid['next_week']['day'] = (int) elk_strftime('%d', $nextWeekTimestamp);
	$calendarGrid['next_week']['month'] = (int) elk_strftime('%m', $nextWeekTimestamp);
	$calendarGrid['next_week']['year'] = (int) elk_strftime('%Y', $nextWeekTimestamp);

	// Fetch the arrays for birthdays, posted events, and holidays.
	$startDate = elk_strftime('%Y-%m-%d', $curTimestamp);
	$endDate = elk_strftime('%Y-%m-%d', $nextWeekTimestamp);
	...
	if (!empty($calendarOptions['show_week_num']))
	{
		...
		// $last_day_of_last_year = (int) elk_strftime('%w', mktime(0, 0, 0, 12, 31, $year - 1));
		...
		$calendarGrid['week_number'] = (int) elk_strftime('%U', mktime(0, 0, 0, $month, $day, $year)) + $nWeekAdjust;

		// If this crosses a year boundary and includes january it should be week one.
		if ((int) elk_strftime('%Y', $curTimestamp + 518400) != $year && $calendarGrid['week_number'] > 53 && $first_day_of_next_year < 5)
	...
	for ($i = 0; $i < 7; $i++)
	{
		...
		$lastDay = $curDay;
		$curTimestamp += 86400;
		$curDay = (int) elk_strftime('%d', $curTimestamp);
...
{
	$low_date = elk_strftime('%Y-%m-%d', forum_time(false) - 24 * 3600);
	$high_date = elk_strftime('%Y-%m-%d', forum_time(false) + $days_to_index * 24 * 3600);
	...
	return array(
		...
		'refresh_eval' => 'return \'' . elk_strftime('%Y%m%d', forum_time(false)) . '\' != elk_strftime(\'%Y%m%d\', forum_time(false)) || (!empty($modSettings[\'calendar_updated\']) && ' . time() . ' < $modSettings[\'calendar_updated\']);',
		'expires' => time() + 3600,
	);
}
...
function cache_getRecentEvents($eventOptions)
{
	...
	{
		if (isset($cached_data['holidays'][elk_strftime('%Y-%m-%d', $i)]))
			$return_data['calendar_holidays'] = array_merge($return_data['calendar_holidays'], $cached_data['holidays'][elk_strftime('%Y-%m-%d', $i)]);
	}

	// Happy Birthday, guys and gals!
	for ($i = $now; $i < $now + $days_for_index; $i += 86400)
	{
		$loop_date = elk_strftime('%Y-%m-%d', $i);
		if (isset($cached_data['birthdays'][$loop_date]))
		{
			foreach ($cached_data['birthdays'][$loop_date] as $index => $dummy)
				$cached_data['birthdays'][elk_strftime('%Y-%m-%d', $i)][$index]['is_today'] = $loop_date === $today['date'];
...
function insertEvent(&$eventOptions)
{
	...
	if (!isset($eventOptions['end_date']))
		$eventOptions['end_date'] = elk_strftime('%Y-%m-%d', mktime(0, 0, 0, $month, $day, $year) + $eventOptions['span'] * 86400);
...
function getEventProperties($event_id, $calendar_only = false)
{
	...
	else
	{
		...
		$return_value['last_day'] = (int) elk_strftime('%d', mktime(0, 0, 0, $return_value['month'] == 12 ? 1 : $return_value['month'] + 1, 0, $return_value['month'] == 12 ? $return_value['year'] + 1 : $return_value['year']));
	}
