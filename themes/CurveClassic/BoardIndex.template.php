				...
				if (!empty($board['children']))
				{
					...
					foreach ($board['children'] as $child)
					{
						if (!$child['is_redirect'])
							$child['link'] = '<a href="' . $child['href'] . '" class="' . ($child['new'] ? 'board_new_posts preview' : 'preview') . '" title="' . $child['description'] . '">' . $child['name'] . ($child['new'] ? '</a> <a href="' . $scripturl . '?action=unread;board=' . $child['id'] . '" class="preview" title="' . $txt['new_posts'] . ' (' . $txt['board_topics'] . ': ' . comma_format($child['topics']) . ', ' . $txt['posts'] . ': ' . comma_format($child['posts']) . ')"><span class="new_posts">' . $txt['new'] . '</span>' : '') . '</a>';
						else
...
function template_info_center()
{
	...
	echo '
			...
			<p class="last">
				...
				', $txt['most_online_ever'], ': ', comma_format($modSettings['mostOnline']), ' (', relativeTime($modSettings['mostDate']), ')
			</p>';
